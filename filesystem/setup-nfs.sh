...
# while kubectl get pvc nfs-pvc -n $NAMESPACE 2> /dev/null; do
...
#     kubectl get pv | grep 'Released\|Bound' | awk '{print $1}' | xargs echo #kubectl delete pv --force --grace-period=0
#     break
#   fi
# done

echo "Applying NFS server service and deployment..."
kubectl apply -f nfs/nfs-service.yaml -n $NAMESPACE
kubectl apply -f nfs/nfs-deployment.yaml -n $NAMESPACE
...
kubectl apply -f nfs/nfs-pv.yaml # Optional
kubectl apply -f nfs/nfs-pvc.yaml -n $NAMESPACE
...
kubectl apply -f zookeeper-statefulset.yaml -n $NAMESPACE
...
echo "Zookeeper Pods:"
kubectl get pods -l app=zookeeper
echo "Persistent Volume and Volume Claim:"
